...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [startImageResponse, endImageResponse] = await Promise.all([
      ai.models.generateImages({ model: 'imagen-4.0-generate-001', prompt: startPrompt, config: commonConfig }),
      ai.models.generateImages({ model: 'imagen-4.0-generate-001', prompt: endPrompt, config: commonConfig })
    ]);
    ...
    return res.status(200).json({ start: `data:image/jpeg;base64,${startBase64}`, end: `data:image/jpeg;base64,${endBase64}` });
  } catch (error) {
