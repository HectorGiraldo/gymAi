...
export class GeminiService {
  ...
  async generateRoutine(days: string, experience: string, goal: string): Promise<Routine> {
              ...
              exercises: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    description: { type: Type.STRING, description: "Descripción detallada de la técnica del ejercicio." }
                  },
    ...
    try {
      const response = await this.ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: "Eres un entrenador personal experto. Tu tarea es crear rutinas de gimnasio detalladas y efectivas. Responde ÚNICAMENTE con un objeto JSON válido que siga el esquema proporcionado. No incluyas texto introductorio, explicaciones o formato markdown como ```json.",
      ...
      if (!parsedResponse.weeklyRoutine || !Array.isArray(parsedResponse.weeklyRoutine)) {
      ...
      throw new Error('No se pudo generar la rutina. Por favor, inténtalo de nuevo.');
    }
  }

  async generateExerciseImages(exerciseName: string): Promise<{ start: string; end: string }> {
    const commonConfig = {
      numberOfImages: 1,
    ...
    try {
      const [startImageResponse, endImageResponse] = await Promise.all([
        this.ai.models.generateImages({ model: 'imagen-4.0-generate-001', prompt: startPrompt, config: commonConfig }),
        this.ai.models.generateImages({ model: 'imagen-4.0-generate-001', prompt: endPrompt, config: commonConfig })
      ]);
